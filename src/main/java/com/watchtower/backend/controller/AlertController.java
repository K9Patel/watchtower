...
public class AlertController {
    ...
    public List<Alert> getActiveAlerts(Authentication authentication) {
        ...
        return alertRepository.findByIsResolvedFalseAndCreatedAtAfterOrderByCreatedAtDesc(scopeStart);
    }
    ...
    public Page<Alert> getAllAlerts(
        ...
        String userEmail = authentication != null ? authentication.getName() : null;
        LocalDateTime scopeStart = userDataScopeService.getOrCreateScopeStart(userEmail);
        return alertRepository.findByCreatedAtAfterOrderByCreatedAtDesc(scopeStart, PageRequest.of(page, size));
    }

    // GET /api/alerts/count — badge number in navbar
    @GetMapping("/count")
    public Map<String, Long> getUnresolvedCount(Authentication authentication) {
        String userEmail = authentication != null ? authentication.getName() : null;
        LocalDateTime scopeStart = userDataScopeService.getOrCreateScopeStart(userEmail);
        return Map.of("unresolved", alertRepository.countByIsResolvedFalseAndCreatedAtAfter(scopeStart));
    }
